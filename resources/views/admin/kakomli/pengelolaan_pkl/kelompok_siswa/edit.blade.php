...
@section('content')
    <title>{{ config('app.name', 'Laravel') }} - Edit Kelompok Siswa PKL</title>
        ...
        <form action="{{ route('admin.kelompok-siswa.update', $kelompok->id) }}" method="POST">
            ...
            <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="dudi_id" id="dudi_id" class="form-select">
                                ...
                                @foreach ($dudi as $item)
                                    <option value="{{ $item->id }}" {{ old('dudi_id', $kelompok->dudi_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            </select>
                            @error('dudi_id')
                            ...
                            <optgroup label="Daftar Anggota Kelompok">
                                @foreach ($anggota as $data)
                                    <option value="{{ $data->id }}"
                                        {{ in_array($data->id, old('anggota', $member)) ? 'selected' : '' }}>
                                        {{ $data->name }}
                                    </option>
                                @endforeach
                            </optgroup>
                ...
                <div class="col-md12 mt-5 text-start">
                    <button type="submit" class="btn btn-primary profile-button">Edit Kelompok</button>
                </div>
        </form>
    </div>
    <script>
