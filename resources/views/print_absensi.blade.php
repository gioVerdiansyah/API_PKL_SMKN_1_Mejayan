...
@section('content')
                ...
                <div class="upload">
                    <div class="d-flex">
                        <div class="upload__info">
                            ...
                            <span class="upload__filename">Generate PDF absensi</span>
                        </div>
                        <button class="upload__button" onclick="generatePdf()">
                            <div class="upload__button__text px-1">
                                <div class="px-3"><i class="fa-solid fa-print"></i></div>
                            </div>
                        </button>
                    </div>
                    <div class="upload__hint">Generating...</div>
    ...
    <script>
        ...
        const generatePdf = () => {
            $.ajax({
                type: "post",
                url: "{{ route('print_absensi', $guru->id) }}",
                data: {
                    guru_id: '{{ $guru->id }}',
                    _token: '{{ csrf_token() }}'
                },
                ...
                success: function(response) {
                    ...
                    if(response.success){
                        ...
                        $('.upload__button__text').html(`
                            <a href="${response.pdf_url}" download="${response.name_file}"><i class="fa-solid fa-download"></i></a>
                        `);
                        ...
                        $('#message-loading').html(`
                            <p style="color: ${response.success ? 'green' : 'red'}">Message: ${response.message}</p>
                            <p style="color: ${response.success ? 'green' : 'red'}">Error: ${response.error}</p>
                        `);
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    // Log pesan error dan status HTTP
                    let errorElements = `
                        <p style="color: red; font-weight: bold">Status: ${textStatus}</p>
                        <p style="color: red">Erorr: ${errorThrown}</p>
                    `;
                    console.log("Error: " + errorThrown);
                    console.log("Status: " + textStatus);
                    // Mengecek jika terdapat responseText
                    if (jqXHR.responseText) {
                        const errorJqXHR = JSON.parse(jqXHR.responseText);
                        console.log("Response Text: " + jqXHR.responseText);
                        errorElements += `<p style="color: red">Reponse: ${errorJqXHR.message}</p>`;
                    }

                    $('#message-loading').html(errorElements);
                }
            });
        }
    </script>
