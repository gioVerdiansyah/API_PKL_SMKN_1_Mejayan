        ...
        <table class="table">
            ...
            <tbody>
                ...
                @foreach ($absensi->groupBy('user_id') as $userId => $userAbsensi)
                    <tr>
                        <td class="border border-3 border-secondary p-2 ps-3">{{ $userAbsensi->first()->user->name }}
                        </td>
                        <td class="border border-3 border-secondary p-2 ps-3">
                            {{ $userAbsensi->first()->user->kelas->kelas }}</td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ $userAbsensi->whereIn('status', ['1', '2', '4', '5'])->count() }}</td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ $userAbsensi->whereIn('status', ['2', '5'])->count() }}</td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ $userAbsensi->where('status', '6')->count() }}</td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ $userAbsensi->whereIn('status', ['4', '5'])->count() }}</td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ \App\Models\Izin::where('user_id', $userId)->where('tipe_izin', 'Sakit')->whereRaw("DATE_FORMAT(created_at, '%m-%Y') = ?", [$bulannya])->count() }}
                        </td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ \App\Models\Izin::where('user_id', $userId)->where('tipe_izin', 'Dispensasi')->whereRaw("DATE_FORMAT(created_at, '%m-%Y') = ?", [$bulannya])->count() }}
                        </td>
                        <td class="border border-3 border-secondary p-2 text-center">
                            {{ $userAbsensi->where('status', '7')->count() }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
